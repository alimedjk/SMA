...
public class Animal extends Objet {
    ...
    public void process(EnvironmentManager environmentManager, Environment environment){
...
//        System.out.println("Explorateur à (" + newLine + ", " + newColumn + "), direction : " + direction);
    ...
    public void moveInDirection(int direction, EnvironmentManager environmentManager, Environment environment) {
        ...
        if (isValidMove(newLine, newColumn)) {
            ...
            if (agent instanceof CommunicatingAgent commAgent) {
                System.out.println("Agent communicant détecté à la position (" + newLine + ", " + newColumn + "). Mouvement annulé.");
                return;
            }
            ...
            if (agent.getHealth() <= 0) {
                ExplorerFactory.setExplorer(agent, environment);
