...
public class ReactiveAgent extends Agent {
    ...
    @Override
    public void process(Treasure treasure, ArenaManager environmentManager, Arena environment) {
    ...
    public void moveToNextZone(ArenaManager environmentManager, Arena environment) {
    ...
    public void moveToNextZoneBis(ArenaManager environmentManager, Arena environment) {
    ...
    public void moveInDirection(int direction, ArenaManager environmentManager, Arena environment) {
        ...
        if (isValidMove(newLine, newColumn)) {
            ...
            // Vérifier si un animal est présent sur ce bloc
                ...
                ArenaElement elementAnimal = Utility.getElementFromBlock(environment, newBlock);
            ...
            ArenaElement element = Utility.getElementFromBlock(environment, newBlock);
            if (element instanceof Treasure treasure) {
                if (!treasure.isCollected() && (Math.random() < 0.25)) {
    ...
    public void moveInDirectionBis(int direction, ArenaManager environmentManager, Arena environment) {
        Block currentBlock = this.getBlock();
        int newLine = currentBlock.getLine();
        int newColumn = currentBlock.getColumn();
    ...
    public void moveToTreasure(Treasure treasure, ArenaManager environmentManager, Arena environment) {
        if (treasure.isCollected()){
//            goToQG();
...
//            System.out.println("Trésor à (" + treasureLine + ", " + treasureColumn + ")");
    ...
    private void randomMove(ArenaManager environmentManager, Arena environment) {
        Block currentBlock = this.getBlock();
        int newLine = currentBlock.getLine();
        int newColumn = currentBlock.getColumn();
